                     ...
                     )
{
    ...
    std::string l_audio_input = g_data->getSettingsValue("Sound/soundInput", std::string("Default"));
    m_in_device = getDeviceNumber(l_audio_input.c_str());

    QStringList l_out_names = getOutputDeviceNames();
    std::string l_audio_output = g_data->getSettingsValue("Sound/soundOutput", std::string("Default"));
